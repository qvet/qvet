...
pub async fn oauth2_initiate(Extension(state): Extension<SharedState>,) -> Json<Oauth2InitiateResponse> {
    ...
    let internal_state = Oauth2FlowState { csrf_token, pkce_verifier };
    let internal_state = hex::encode(serde_json::to_string(&internal_state).expect("json failure"));
    // This is the URL you should redirect the user to, in order to trigger the authorization
    // process.
    Json(Oauth2InitiateResponse{redirect_url: authorize_url, internal_state})
}
...
pub async fn oauth2_callback(
    ...
    let internal_state: Oauth2FlowState = serde_json::from_slice(&hex::decode(payload.internal_state).expect("hex failure")).expect("json failure");
    let token_result = &state.oauth2_client
        .exchange_code(payload.code)
        // Set the PKCE code verifier.
        .set_pkce_verifier(internal_state.pkce_verifier)
        .request_async(async_http_client)
        .await.expect("request failure");

    Json(Oauth2CallbackResponse { access_token: token_result.access_token().to_owned() })
}
